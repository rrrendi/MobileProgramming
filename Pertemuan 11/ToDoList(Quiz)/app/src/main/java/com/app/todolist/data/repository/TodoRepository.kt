...
class TodoRepository {
    ...
    // [UBAH] Menambahkan parameter category
    suspend fun addTodo(userId: String, title: String, priority: String, category: String) {
        val todo = Todo(title = title, priority = priority, category = category)
    ...
    suspend fun updateTodoCategory(userId: String, todoId: String, newCategory: String) {
        getTodoCollection(userId).document(todoId).update("category", newCategory).await()
    }
